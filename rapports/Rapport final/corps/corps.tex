...
Le jeu se base sur le fonctionnement du célèbre jeu \emph{Agar.io}. Le joueur commence la partie avec le navire de plus bas niveau (un bâtiment-école de niveau 1). Le joueur va devoir trouver puis tirer sur les ennemis afin d'engranger de l'expérience en endommageant des navires ennemie ou en les coulant (il obtient un bonus proportionnel au niveau(tier) du navire coulé).\\	Après un certain palier d'expérience il monte de niveau, afin de manoeuvrer un navire de plus en plus performant. les niveaux vont de 1 à 5. Après 50000 exp le joueur est déclaré vainqueur. Le joueur possède un nombre de points de vie, et il peut mourir si ce nombre tombe à 0. Dans ce cas, le joueur revis dans un bâtiment du niveau inférieur, et son exp est réinitialisé au palier inférieur. Le joueur peut notamment consulter son expérience actuelle via une commande dans une chatbox.\\
...
\begin{figure}[H]
	...
	\caption{Diagramme de comportement de l'IA (il se peut que certaines fonctionnalités ne soit pas encore implémentés).}
	\label{fig:FSM}
\end{figure}
