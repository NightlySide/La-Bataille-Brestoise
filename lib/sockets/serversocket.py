...
class ServerMessage:
    ...
    def _write(self):
        if self._send_buffer:
            ...
            try:
                ...
                # Fermé quand le buffer est drainé. La réponse a été envoyée..
    ...
    def _create_response_binary_content(self):
        response = {
            "content_bytes": b"10 premiers bytes de la requete: "
            + self.request[:10],
    ...
    def close(self):
        print("Fermeture de la connexion avec : ", self.addr)
        try:
    ...
    def process_request(self):
        ...
        if self.jsonheader["content-type"] == "text/json":
            ...
            print("Requête reçue ", repr(self.request), " de ", self.addr)
        else:
            # Binaire ou type de contenu non reconnu
            self.request = data
            print(
                f'Reçu {self.jsonheader["content-type"]} comme requete de ',
                self.addr,
            )
        # On définit le selecteur sur les évènements d'écriture, on a finit de lire.
